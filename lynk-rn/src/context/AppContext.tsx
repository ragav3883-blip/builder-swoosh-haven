...
export const AppProvider: React.FC<React.PropsWithChildren> = ({
    ...
    (async () => {
      ...
      if (raw) {
        try {
          ...
          if (loaded) {
            if (loaded.currentCity)
              dispatch({ type: "SET_CITY", city: loaded.currentCity });
            if (loaded.language)
              dispatch({ type: "SET_LANG", lang: loaded.language });
            if (loaded.favorites)
              loaded.favorites.forEach((id) =>
  ...
  const data = useMemo(
    () => (state.currentCity === "Punjab" ? punjabData : chennaiData),
    [state.currentCity],
  );

  return (
    <AppCtx.Provider value={{ state, dispatch, data }}>
      {children}
    </AppCtx.Provider>
  );
};
