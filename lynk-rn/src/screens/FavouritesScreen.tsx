...
export default function FavouritesScreen() {
  const { state } = useApp();
  const allRoutes = useMemo(() => state.currentCity === 'Punjab' ? [] : [], [state.currentCity]);
  ...
  const favoriteRoutes = [...chennai, ...punjab].filter((r: any) => favIds.includes(r.id));
    ...
    <ScrollView style={{ flex: 1, backgroundColor: '#F3F4F6' }} contentContainerStyle={{ padding: 20 }}>
      ...
      {favoriteRoutes.length > 0 ? (
        ...
        <View style={{ alignItems: 'center', marginTop: 80 }}>
          <Text style={{ color: '#6B7280' }}>Your saved routes will appear here.</Text>
        </View>
      )}
    </ScrollView>
  );
